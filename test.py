...
class Visualizer(object):
    ...
    def __init__(self, path):
        ...
        self.w.setCameraPosition(pos=self.Vector(
            0, 500, 100), distance=600, azimuth=-45, elevation=0)
        self.w.show()

        # Primary feature extraction from the audio
        self.time_series, self.sample_rate = librosa.load(
            './audio/Drake-Papis Home.mp3')
        self.hop_length = 512
    ...
    def initialize_visuals(self):
        ...
        for i in range(self.window_length):
            pts = self.specto_beat_sample[start_pt:start_pt +
    ...
    def update(self):
        ...
        self.w.grabFrameBuffer().save(os.path.join(
            self.img_path, f'img_{tracker_id}.png'))
    ...
    def get_spectrogram(self):
        ...
        spectrogram = librosa.amplitude_to_db(
            self.stft, ref=np.max)  # Spectrogram
